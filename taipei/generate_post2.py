"""Generate Instagram-ready image carousel (V2 - "A24 Style" + Spread Layout) for Taipei.
...
- Timezone: Taipei (UTC+8), 3-day film rotation history.
...
def get_today_taipei():
    """Returns the current datetime in Taipei."""
    return datetime.now(TAIPEI_TZ)

def get_today_str():
    """Returns YYYY-MM-DD in Taipei."""
    return get_today_taipei().strftime("%Y-%m-%d")

def get_display_date_str():
    """Returns formatted date string for display (Taipei)."""
    d = get_today_taipei()
...
def draw_poster_slide(film, img_obj, fonts, is_story=False, primary_date=None):
    ...
    available_space = height - cursor_y - 40
    base_line_h = 28
    ...
    if not needs_split:
        ...
        if est_h > available_space:
            scale = max(available_space / est_h, 0.8)
        ...
        max_height = available_space - 20
        start_y = cursor_y + 10
        displayed_cinemas = []
        current_height = 0

        for cin in sorted_cinemas:
            needed = line_height_cinema + len(schedule_map[cin]) * line_height_showtime + gap_between_cinemas
...
def main():
    print("--- Starting Taipei Today's Film Selection (V2) ---")
    ...
    today_dt = get_today_taipei().date()
    date_strs = [
    ...
    caption_lines = [f"🎬 Taipei Today's Film Selection - {get_display_date_str()}\n"]
